...
const safeParseUnits = (v: string, decimals = 18) => {
  try {
    return parseUnits(v && v.trim() !== "" ? v : "0", decimals);
  } catch {
...
export default function ComprehensiveAdminPanel() {
  ...
  const fetchAdminData = async () => {
    ...
    try {
      ...
      const normalizedRes = await fetch(
        API_ENDPOINTS.GET_ADMIN_OVERVIEW_NORMALIZED,
      );
  ...
  const backAnalytics = adminAnalytics || {
    totalUsers: 0,
    totalVolume: "0",
    totalWithdrawn: "0",
  };
    ...
    : {
        users: userStats
          ? Array.isArray(userStats)
            ? String(userStats[0])
            : String((userStats as any).users)
          : backAnalytics.totalUsers,
        volume: userStats
          ? Array.isArray(userStats)
            ? String(userStats[1])
            : String((userStats as any).volume)
          : backAnalytics.totalVolume,
        withdrawn: userStats
          ? Array.isArray(userStats)
            ? String(userStats[2])
            : String((userStats as any).withdrawn)
          : backAnalytics.totalWithdrawn,
      };
  ...
  const poolStats = normalizedPools
    ...
    : {
        gamePool: poolStatsRaw
          ? fmt(
              Array.isArray(poolStatsRaw)
                ? poolStatsRaw[0]
                : (poolStatsRaw as any).gamePool,
            )
          : "0.00",
        clubPool: poolStatsRaw
          ? fmt(
              Array.isArray(poolStatsRaw)
                ? poolStatsRaw[1]
          ...
          : "0.00",
        goldenPool: poolStatsRaw
          ? fmt(
              Array.isArray(poolStatsRaw)
                ? poolStatsRaw[2]
                : (poolStatsRaw as any).golden,
            )
          : "0.00",
        silverPool: poolStatsRaw
          ? fmt(
              Array.isArray(poolStatsRaw)
                ? poolStatsRaw[3]
                : (poolStatsRaw as any).silver,
            )
          : "0.00",
        protectionPool: poolStatsRaw
          ? fmt(
              Array.isArray(poolStatsRaw)
                ? poolStatsRaw[4]
                : (poolStatsRaw as any).protection,
            )
          : "0.00",
        creatorPool: poolStatsRaw
          ? fmt(
              Array.isArray(poolStatsRaw)
                ? poolStatsRaw[5]
                : (poolStatsRaw as any).creator,
            )
          : "0.00",
        bdPool: poolStatsRaw
          ? fmt(
              Array.isArray(poolStatsRaw)
                ? poolStatsRaw[6]
                : (poolStatsRaw as any).bd,
            )
          : "0.00",
        fewPool: poolStatsRaw
          ? fmt(
              Array.isArray(poolStatsRaw)
                ? poolStatsRaw[7]
                : (poolStatsRaw as any).few,
            )
          : "0.00",
        referralPool: poolStatsRaw
          ? fmt(
              Array.isArray(poolStatsRaw)
                ? poolStatsRaw[8]
                : (poolStatsRaw as any).referral,
            )
          : "0.00",
      };
    ...
    : {
        practiceId:
          currentPracticeId != null
            ...
            : adminGameStatus?.practiceId || "0",
        cashId:
          currentCashId != null
            ? String(currentCashId)
            : adminGameStatus?.cashId || "0",
      };
...
function OverviewTab({
  ...
  const volume = stats?.volume
    ? Number(formatUnits(BigInt(stats.volume), 18)).toFixed(2)
    : stats?.[1]
    ? Number(formatUnits(BigInt(stats[1]), 18)).toFixed(2)
    : "0.00";
  const withdrawn = stats?.withdrawn
    ? Number(formatUnits(BigInt(stats.withdrawn), 18)).toFixed(2)
    : stats?.[2]
    ? Number(formatUnits(BigInt(stats[2]), 18)).toFixed(2)
    : "0.00";
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
        ...
        <QuickStat
          label="Silver Draw"
          value={poolStats.silverPool}
          sub="USDT"
          color="text-gray-400"
        />

        <QuickStat
          label="Creator Distributed"
          value={poolStats.creatorPool}
          sub="USDT (Cumulative)"
          color="text-pink-400"
        />
        <QuickStat
          label="BD Distributed"
          value={poolStats.bdPool}
          sub="USDT (Cumulative)"
          color="text-indigo-400"
        />
        <QuickStat
          label="Ops/FEW Distributed"
      ...
      <div className="bg-blue-950/20 border border-blue-500/20 rounded-xl p-4 text-xs text-blue-200">
        ...
        <p>
          Protection is a reserve used for loss-cashback and safety payouts. It
          helps absorb downside during weak rounds and supports payout
          stability.
        </p>
      </div>
      ...
      <div className="bg-black/40 border border-white/5 rounded-2xl p-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-sm">
          <div className="border-2 border-yellow-500 rounded-xl p-3 shadow-[0_0_15px_rgba(234,179,8,0.2)]">
            <p className="text-yellow-500 font-black uppercase text-[10px] mb-1">
              🔥 Current Minimum
            </p>
            <p className="font-mono font-black text-lg">
              {settingsStr?.[0] ?? "1.00"} USDT
            </p>
          </div>
          <div>
            <p className="text-gray-500">Sign-up Bonus</p>
            <p className="font-mono font-bold">
              {settingsStr?.[1] ?? "10.00"} USDT
            </p>
          </div>
          <div>
          ...
          <div>
            <p className="text-gray-500 font-bold uppercase text-[9px]">
              Min Active Deposit For Referral Payout
            </p>
            <p className="font-mono font-bold">
              {settingsStr?.[5] ?? "0.00"} USDT
            </p>
            <p className="text-[8px] text-gray-600 italic">
              Referral income is released only after this active deposit
              threshold is met.
            </p>
          </div>
        </div>
...
function LuckyDrawCard({
  ...
  // type 1 = Golden (10 USDT, 70k pool), type 0 = Silver (1 USDT, 7k pool)
  const prizes =
    type === 1
      ? [
  ...
  return (
    <div
      className={`relative overflow-hidden bg-gradient-to-br ${colorClasses} border rounded-2xl p-6`}
    >
      <div
        className={`absolute inset-0 ${
          baseColor === "pink" ? "bg-pink-500/5" : "bg-blue-500/5"
        } blur-3xl -z-10`}
      />
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">
            Ticket Price: {type === 1 ? "10" : "1"} USDT
          </p>
        </div>
        <div className="text-right">
          <p className={`text-sm font-mono`}>
            Prize Pool:{" "}
            <span className="text-white font-bold">${prizePoolUSDT} USDT</span>
          </p>
        </div>
      </div>
          ...
          <div
            className={`h-full bg-gradient-to-r ${
              baseColor === "pink"
                ...
                : "from-blue-600 to-gray-500"
            } rounded-full transition-all duration-700 ease-out`}
            style={{ width: `${Math.max(pct, ticketsSold > 0 ? 2 : 0)}%` }}
          />
        </div>
        <p className="text-center text-xs text-pink-400 mt-1 font-mono">
          {pct}% filled
        </p>
      </div>
      ...
      >
        {isPending
          ...
          : ticketsSold === 0
          ? "⚠️ No Tickets"
          : `🎰 Force Draw #${drawId} (${ticketsSold} tix)`}
      </button>
    </div>
  );
}
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex bg-white/5 p-1 rounded-2xl border border-white/10 w-full sm:w-auto overflow-hidden">
          <button
            ...
            className={`flex-1 sm:px-8 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
      ...
      <motion.div
        ...
        className={`bg-white/[0.02] backdrop-blur-3xl border-2 p-8 md:p-12 rounded-[2.5rem] text-center ${
          isCash
            ? "border-yellow-500/30 shadow-[0_0_50px_rgba(234,179,8,0.1)]"
            : "border-blue-500/30 shadow-[0_0_50px_rgba(59,130,246,0.1)]"
        }`}
      >
        <div className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400/40 mb-4">
          {isCash ? "Global Cash Arena" : "Demo Practice Lab"}
        </div>
          ...
          <div className="relative">
            <div
              className={`absolute inset-0 blur-2xl opacity-20 ${
                isCash ? "bg-yellow-500" : "bg-blue-500"
              }`}
            ></div>
            <div
              className={`relative text-7xl md:text-9xl font-black tracking-tighter ${
                isCash ? "text-yellow-400" : "text-blue-400"
              }`}
            >
        ...
        <div className="max-w-md mx-auto space-y-6">
          <div className="relative group">
            ...
            <input
              ...
              placeholder="0-9"
            />
            <div className="text-[10px] text-gray-600 font-bold uppercase tracking-widest mt-2">
              {isCash ? "Inject Cash Outcome" : "Set Demo Outcome"}
            </div>
          </div>
          ...
          <button
            ...
            className={`w-full py-5 rounded-2xl font-black text-lg transition-all active:scale-95 shadow-2xl disabled:opacity-30 ${
              isCash
          ...
          <p className="text-[9px] text-gray-600 font-medium leading-relaxed italic uppercase">
            Caution: This action acts as the final oracle. Payouts are
            irreversible on the BSC ledger.
          </p>
        </div>
      </motion.div>
    </div>
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex bg-white/5 p-1 rounded-2xl border border-white/10 w-full md:w-auto overflow-hidden">
          <button
            ...
            className={`flex-1 md:px-8 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
              activeSubTab === "system"
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {activeSubTab === "system" ? (
            <SectionCard
              title="Registration & Bonus Settings"
              onSave={handleUpdateRouterSettings}
              isPending={isPending}
            >
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
                <InputGroup
                  label="Min Activation (USDT)"
                  value={form.minActivation}
                  onChange={(v: any) => updateField("minActivation", v)}
                />
                <InputGroup
                  label="Signup Bonus Tier 1 (USDT)"
                  value={form.signupBonusTier1}
                  onChange={(v: any) => updateField("signupBonusTier1", v)}
                />
                <InputGroup
                  label="Tier 1 User Limit"
                  value={form.tier1Limit}
                  onChange={(v: any) => updateField("tier1Limit", v)}
                />
                <InputGroup
                  label="Signup Bonus Tier 2 (USDT)"
                  value={form.signupBonusTier2}
                  onChange={(v: any) => updateField("signupBonusTier2", v)}
                />
                <InputGroup
                  label="Tier 2 User Limit"
                  value={form.tier2Limit}
                  onChange={(v: any) => updateField("tier2Limit", v)}
                />
                <InputGroup
                  label="Min Ref Payout (USDT)"
                  value={form.minReferral}
                  onChange={(v: any) => updateField("minReferral", v)}
                />
                <div className="bg-blue-500/10 p-4 rounded-2xl border border-blue-500/20">
                  <InputGroup
                    label="Full Activation Bridge (USDT)"
                    value={form.fullBridge}
                    onChange={(v: any) => updateField("fullBridge", v)}
                  />
                  <p className="text-[8px] text-blue-400 mt-2 font-bold uppercase tracking-tighter italic">
                    Amount needed to convert Practice Balance to Real Balance
                  </p>
                </div>
              </div>
              <div className="mt-8 p-4 bg-white/5 rounded-2xl border border-white/10 flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-blue-500/20 flex items-center justify-center text-blue-400 text-xs text-center">
                  ℹ️
                </div>
                <p className="text-[10px] text-gray-500 font-medium">
                  These settings govern account activation and initial bonus
                  eligibility across the platform. Changes are applied via the
                  Router contract.
                </p>
              </div>
            </SectionCard>
            ...
            <SectionCard
              title="Gameplay & Engine Limits"
              onSave={handleUpdateGameEngine}
              isPending={isPending}
            >
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                <InputGroup
                  label="Practice Games / Day"
                  value={form.practiceGames}
                  onChange={(v: any) => updateField("practiceGames", v)}
                />
                <InputGroup
                  label="Cash Games / Day"
                  value={form.cashGames}
                  onChange={(v: any) => updateField("cashGames", v)}
                />
              </div>
              <div className="mt-8 p-4 bg-white/5 rounded-2xl border border-white/10 flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-green-500/20 flex items-center justify-center text-green-400 text-xs text-center">
                  ⚙️
                </div>
                <p className="text-[10px] text-gray-500 font-medium">
                  These limits are stored directly on the Game Engine contract
                  for high-performance throughput and distinct access control.
                </p>
              </div>
            </SectionCard>
          )}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-yellow-900/20 border border-yellow-500/30 p-4 rounded-lg text-center">
        <p className="text-2xl font-bold">
          Total Distributed: <span className="text-green-400">{total}%</span>
        </p>
...
function WalletsTab({ writeContract, isPending }: any) {
  ...
  const [bdWallets, setBdWallets] = useState<string[]>(
    Array(24).fill(defaultAddr),
  );
  ...
  useEffect(() => {
    if (walletsData) {
      ...
      setFewWallet(
        few && few !== "0x0000000000000000000000000000000000000000"
          ? few
          : defaultAddr,
      );
  ...
  const handleUpdate = () => {
    if (isPending) return;
    const safeBdWallets = bdWallets.map((addr) =>
      addr && addr.trim() !== "" ? addr : defaultAddr,
    );
    ...
    writeContract({
      ...
      args: [
        creatorWallet || defaultAddr,
        fewWallet || defaultAddr,
        safeBdWallets,
      ],
    });
  };
...
export function TransactionsTab({ filter, setFilter }: any) {
  ...
  useEffect(() => {
    const fetchHistoryFromBackend = async () => {
      ...
      try {
        ...
        if (data.success) {
          const combined = [
            ...data.deposits.map((tx: any) => ({
              ...tx,
              type: "deposit",
              functionName: "Deposit USDT",
            })),
            ...data.withdrawals.map((tx: any) => ({
              ...tx,
              type: "withdrawal",
              functionName: "Withdraw USDT",
              value: tx.totalRequested,
            })),
            ...data.luckyTickets.map((tx: any) => ({
              ...tx,
              type: "lucky",
              functionName: `Buy Lucky Tickets (${
                tx.drawType === 1 ? "Golden" : "Silver"
              })`,
              value: parseUnits(
                (tx.count * (tx.drawType === 1 ? 10 : 1)).toString(),
                18,
              ).toString(),
            })),
            ...data.bets.map((tx: any) => ({
              ...tx,
              type: "bet",
              functionName: `Place Bet (${tx.isCash ? "Cash" : "Practice"})`,
            })),
            ...data.winnings.map((tx: any) => ({
              ...tx,
              type: "win",
              functionName: "Claim Win",
            })),
            ...data.incomes.map((tx: any) => ({
              ...tx,
              type: "income",
              functionName: `Income: ${tx.source}`,
            })),
            ...(data.conversions || []).map((tx: any) => ({
              ...tx,
              type: "conversion",
              functionName: "Convert Practice to Cash",
              value: tx.amount,
            })),
            ...(data.rewards || []).map((tx: any) => ({
              ...tx,
              type: "reward",
              functionName: `Practice Reward (L${tx.level})`,
              value: tx.amount,
            })),
          ];
        ...
        setDebugMsg(
          err.message ||
            "Failed to fetch data from backend. Make sure the backend server is running.",
        );
      } finally {
        setLoading(false);
      }
  ...
  const filteredTx = transactions.filter((tx) => {
    ...
    const timeOk =
      timeWindow === "all" ? true : Number(tx.timestamp || 0) >= windowStart;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
        <div>
          ...
          <p className="text-gray-400 text-sm">
            ...
            <span className="text-blue-400 font-mono">Backend Database</span>
          </p>
        </div>
              ...
              {loading && transactions.length === 0 ? (
                ...
                filteredTx.map((tx, idx) => (
                  <tr key={`${tx.hash}-${idx}`} className="hover:bg-white/5">
                    <td className="p-4">
                      <span
                        className={`text-[10px] border px-2 py-0.5 rounded uppercase font-black ${
                          tx.type === "deposit"
                    ...
                    <td className="p-4 font-mono text-xs text-gray-500">
                      {tx.userAddress?.slice(0, 6)}...
                      {tx.userAddress?.slice(-4)}
                    </td>
                    <td className="p-4 text-right font-mono text-xs font-bold">
                      {tx.amount
                        ? Number(formatUnits(BigInt(tx.amount), 18)).toFixed(2)
                        : tx.value
                        ? Number(formatUnits(BigInt(tx.value), 18)).toFixed(2)
                        : "-"}
                    </td>
                  </tr>
                ))
...
function TreasuryEconomics({
  ...
  const [refPercents, setRefPercents] = useState<string[]>(() =>
    refs
      ? Array.from({ length: 15 }, (_, i) =>
          (Number(refs[i] ?? 0) / 100).toString(),
        )
      : Array(15).fill("0"),
  );

  useEffect(() => {
    setForm(makeForm(params));
  }, [params]);
  useEffect(() => {
    if (refs)
      setRefPercents(
  ...
  const handleUpdateTreasury = () => {
    try {
      writeContract({
        ...
        args: [
          safeParseUnits(form.minA, 18),
          safeParseUnits(form.minW, 18),
          safeParseUnits(form.maxW, 18),
          safeBigInt(form.fee),
        ],
      });
    } catch (e) {
  ...
  const handleUpdateRefs = () => {
    try {
      const padded = safePad15(
        refPercents.map((p) => Math.round(Number(p || 0) * 100)),
      );
      writeContract({
        address: TRK_GAME_ADDRESS,
        abi: TRKGameABI.abi,
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <InputGroup
            ...
            value={form.minA}
            onChange={(v: any) => setForm({ ...form, minA: v })}
          />
          <InputGroup
            label="Min Withdrawal"
            value={form.minW}
            onChange={(v: any) => setForm({ ...form, minW: v })}
          />
          <InputGroup
            label="Max Daily Withdr"
            value={form.maxW}
            onChange={(v: any) => setForm({ ...form, maxW: v })}
          />
          <InputGroup
            label="Withdraw Fee (%)"
            value={form.fee}
            onChange={(v: any) => setForm({ ...form, fee: v })}
          />
        </div>
      </SectionCard>

      <SectionCard
        title="Ecosystem Distributions (%)"
        onSave={handleUpdateDist}
        isPending={isPending}
      >
        <div className="grid grid-cols-2 lg:grid-cols-4 xl:grid-cols-7 gap-4">
          <InputGroup
            label="Creator"
            value={form.cP}
            onChange={(v: any) => setForm({ ...form, cP: v })}
          />
          <InputGroup
            label="BD Org"
            value={form.bdP}
            onChange={(v: any) => setForm({ ...form, bdP: v })}
          />
          <InputGroup
            label="Ops/FEW"
            value={form.fwP}
            onChange={(v: any) => setForm({ ...form, fwP: v })}
          />
          <InputGroup
            label="Referral"
            value={form.rfP}
            onChange={(v: any) => setForm({ ...form, rfP: v })}
          />
          <InputGroup
            label="Club"
            value={form.clP}
            onChange={(v: any) => setForm({ ...form, clP: v })}
          />
          <InputGroup
            label="Lucky"
            value={form.lkP}
            onChange={(v: any) => setForm({ ...form, lkP: v })}
          />
          <InputGroup
            label="Protection"
            value={form.ptP}
            onChange={(v: any) => setForm({ ...form, ptP: v })}
          />
        </div>
      </SectionCard>

      <SectionCard
        title="Direct Referral Levels (%)"
        onSave={handleUpdateRefs}
        isPending={isPending}
      >
...
function CashbackEconomics({
  ...
  const [roiPercents, setRoiPercents] = useState<string[]>(() =>
    rois
      ? Array.from({ length: 15 }, (_, i) => (rois[i] ?? 0).toString())
      : Array(15).fill("0"),
  );
  ...
  return (
    <div className="space-y-8">
      <SectionCard
        title="Immediate Loss Protection & ROI"
        onSave={handleUpdateCashback}
        isPending={isPending}
      >
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          <InputGroup
            label="Loss Cashback (%)"
            value={form.lcBps}
            onChange={(v: any) => setForm({ ...form, lcBps: v })}
          />
          <InputGroup
            label="Minimum Net Loss (USDT)"
            value={form.threshold}
            onChange={(v: any) => setForm({ ...form, threshold: v })}
          />
          <div className="bg-pink-500/10 p-4 rounded-xl border border-pink-500/20">
            <InputGroup
              label="Lucky Share (%)"
              value={form.luckyS}
              onChange={(v: any) => setForm({ ...form, luckyS: v })}
            />
            <p className="text-[10px] text-pink-400 mt-2 font-bold uppercase tracking-tighter">
              Pool Split
            </p>
          </div>
          <InputGroup
            label="ROI Pool Ratio (%)"
            value={form.roiR}
            onChange={(v: any) => setForm({ ...form, roiR: v })}
          />
          <div className="bg-blue-500/10 p-4 rounded-xl border border-blue-500/20">
            <InputGroup
              label="Max Daily Cashback (USDT)"
              value={form.maxDaily}
              onChange={(v: any) => setForm({ ...form, maxDaily: v })}
            />
      ...
      <SectionCard
        title="Sustainable ROI Levels (%)"
        onSave={handleUpdateRoiRefs}
        isPending={isPending}
      >
        <div className="grid grid-cols-3 md:grid-cols-5 gap-4">
      ...
      >
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          <InputGroup
            label="Win Cashout (X)"
            value={form.wc}
            onChange={(v: any) => setForm({ ...form, wc: v })}
          />
          <InputGroup
            label="Win Reinvest (X)"
            value={form.wr}
            onChange={(v: any) => setForm({ ...form, wr: v })}
          />
          <InputGroup
            label="Practice Limit / Day"
            value={form.pl}
            onChange={(v: any) => setForm({ ...form, pl: v })}
          />
          <InputGroup
            label="Cash Limit / Day"
            value={form.cl}
            onChange={(v: any) => setForm({ ...form, cl: v })}
          />
        </div>
        <p className="text-xs text-gray-500 mt-4">
          Total Win Payout is {Number(form.wc) + Number(form.wr)}X. Whitepaper
          standard is 8X (2X Cash + 6X Re-bet).
        </p>
      </SectionCard>

      <SectionCard
        title="Winner Referral Levels (%)"
        onSave={handleUpdateWinRefs}
        isPending={isPending}
      >
        <div className="grid grid-cols-3 md:grid-cols-5 gap-4">
          {refPercents.map((p: string, i: number) => (
...
function LuckyEconomics({ params, writeContract, isPending, luckyAddr }: any) {
  ...
  const handleUpdateLucky = () => {
    try {
      writeContract({
        ...
        args: [
          safeBigInt(form.mt),
          safeParseUnits(form.gp, 18),
          safeParseUnits(form.sp, 18),
        ],
      });
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="space-y-8">
      <SectionCard
        title="Lucky Draw Parameters"
        onSave={handleUpdateLucky}
        isPending={isPending}
      >
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <InputGroup
            label="Max Tickets (per draw)"
            value={form.mt}
            onChange={(v: any) => setForm({ ...form, mt: v })}
          />
          <InputGroup
            label="Golden Price (USDT)"
            value={form.gp}
            onChange={(v: any) => setForm({ ...form, gp: v })}
          />
          <InputGroup
            label="Silver Price (USDT)"
            value={form.sp}
            onChange={(v: any) => setForm({ ...form, sp: v })}
          />
        </div>
      </SectionCard>
      ...
      <div className="bg-yellow-900/10 border border-yellow-500/20 p-6 rounded-2xl">
        ...
        <p className="text-sm text-gray-400">
          Prize Amounts for all 8 tiers of Golden & Silver draws can be updated
          via specific contract functions if needed. Currently using standard
          whitepaper ratios.
        </p>
      </div>
    </div>
  ...
  const handleSave = async () => {
    try {
      ...
      const capsBeforeArr = [
        {
          directs: BigInt(20),
          multiplier: BigInt(settings.before[20]) / BigInt(100),
        },
        {
          directs: BigInt(10),
          multiplier: BigInt(settings.before[10]) / BigInt(100),
        },
        {
          directs: BigInt(5),
          multiplier: BigInt(settings.before[5]) / BigInt(100),
        },
      ...
      const capsAfterArr = [
        {
          directs: BigInt(20),
          multiplier: BigInt(settings.after[20]) / BigInt(100),
        },
        {
          directs: BigInt(10),
          multiplier: BigInt(settings.after[10]) / BigInt(100),
        },
        {
          directs: BigInt(5),
          multiplier: BigInt(settings.after[5]) / BigInt(100),
        },
        { directs: BigInt(0), multiplier: BigInt(1) },
      ];
      writeContract({
      ...
      alert(
        "Transaction(s) submitted to blockchain! Confirm up to 3 transactions in sequence.",
      );
    } catch (e) {
      console.error(e);
  ...
  return (
    <div className="space-y-8 pb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                placeholder="10000"
              />
              <p className="text-[10px] text-gray-500 mt-2 px-1 italic font-medium">
                When the platform reaches this many users, the 'After Milestone'
                caps will apply.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center justify-center p-6 bg-yellow-500/5 rounded-3xl border border-yellow-500/10">
            <div className="text-center">
              <div className="text-4xl font-black text-yellow-500 mb-1 leading-none">
                {settings.threshold.toLocaleString()}
              </div>
              <div className="text-[10px] font-black uppercase tracking-widest text-yellow-500/60 mt-1">
                Target Users
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            {[5, 10, 20].map((tier) => (
              <div key={tier} className="flex items-center gap-4 group">
                <div className="w-24 text-[10px] font-black text-gray-500 uppercase tracking-widest group-hover:text-blue-400 transition-colors">
                  {tier} Directs:
                </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl p-8 rounded-[2.5rem] border border-white/10 shadow-2xl transition-all hover:bg-white/[0.07]">
          ...
          <div className="space-y-6">
            {[5, 10, 20].map((tier) => (
              <div key={tier} className="flex items-center gap-4 group">
                <div className="w-24 text-[10px] font-black text-gray-500 uppercase tracking-widest group-hover:text-pink-400 transition-colors">
                  {tier} Directs:
                </div>
                <div className="relative flex-1">
                  <input
      ...
      <div className="pt-4">
        ...
        >
          <span>💾</span>{" "}
          {isPending ? "SENDING TO BLOCKCHAIN..." : "SAVE CAP SETTINGS"}
        </button>
