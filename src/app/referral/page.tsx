...
const COMMISSION_RATES = [
  5, 2, 1, 1, 1, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5, 0.5,
];
      ...
      <div className="mb-2 relative">
          ...
          className={`p-3 rounded-xl border flex justify-between items-center transition-all group
                    ${
                      level === 0
                        ...
                        : level === 1
                        ? "bg-black/20 border-gray-800 ml-4"
        ...
        >
            ...
            <div className="min-w-0">
              ...
              {hasLoadedCount && (
                <p className="text-[10px] text-gray-500 uppercase tracking-wider">
                  Sponsoring:{" "}
                  <span className="text-white font-bold">{childCount}</span>
                </p>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {childCount > 0 && (
              <div
                className={`w-6 text-center text-primary font-bold transform transition-transform ${
        ...
        <AnimatePresence>
          {isExpanded && (
            ...
            >
              {loading ? (
                <p className="text-xs text-gray-500 p-4 animate-pulse">
                  Scanning blockchain...
                </p>
              ) : (
                <div className="pt-2">
...
function UserDetailModal({
  ...
  const totalDeposit = user?.cumulativeDeposit
    ? Number(formatUnits(user.cumulativeDeposit, 18))
    : 0;
    ...
    >
      ...
      >
        <div className="p-6">
          ...
          {isLoading || !user ? (
            <div className="py-12 text-center animate-pulse text-primary">
              Loading Data...
            </div>
          ) : (
...
export default function ReferralPage() {
  ...
  const [networkStats, setNetworkStats] = useState<{
    [lvl: number]: { count: number; bonus: bigint };
  }>({});
  ...
  const isRegistered: boolean =
    user?.isRegistered ?? (userData as any[])?.[28] ?? false;
  const directReferralCount: number = user?.directReferrals
    ? Number(user.directReferrals)
    : (userData as any[])?.[31]
    ? Number((userData as any[])?.[31])
    : 0;
  const cumulativeDeposit: bigint =
    user?.cumulativeDeposit ?? (userData as any[])?.[8] ?? BigInt(0);
  ...
  const dIncomeVal =
    user?.directReferralIncome ?? (userData as any[])?.[9] ?? BigInt(0);
  const wIncomeVal =
    user?.winnerReferralIncome ?? (userData as any[])?.[10] ?? BigInt(0);
  const pIncomeVal =
    user?.practiceReferralIncome ?? (userData as any[])?.[11] ?? BigInt(0);
  const totalDepositVal: bigint =
    user?.totalDeposit ?? (userData as any[])?.[6] ?? BigInt(0);
    ...
    async function fetchTeamBalances() {
      try {
        const res = await fetch(
          API_ENDPOINTS.GET_TEAM_STATS(address as string),
        );
        const data = await res.json();
    ...
    <div className="container mx-auto px-4 py-12 max-w-6xl">
      ...
      {!isEligible && (
        <div className="bg-blue-500/10 border border-blue-500/50 p-4 rounded-xl mb-8 text-center text-sm">
          ...
          <p className="text-gray-400">
            Deposit {formatUnits(minReferralPayout, 18)} USDT to unlock real
            commissions.
          </p>
        </div>
      )}
      ...
      <div className="bg-surface/50 p-8 rounded-2xl border border-gray-800 mb-8 backdrop-blur-md">
        ...
        <p className="text-[10px] text-gray-500 mt-4 text-center italic">
          Share this code with your network to earn from 15 levels of depth.
        </p>
      </div>
      ...
      <div className="bg-gray-900/40 p-6 md:p-8 rounded-3xl border border-gray-800 backdrop-blur-xl mb-8">
          ...
          <table className="w-full text-left border-separate border-spacing-y-2">
            ...
            <tbody>
              {Array.from({ length: 15 }).map((_, i) => {
                ...
                const directArray =
                  user?.directReferralIncomeByLevel ||
                  (userData as any[])?.[17];
                const winnerArray =
                  user?.winnerReferralIncomeByLevel ||
                  (userData as any[])?.[18];
                const practiceArrayFromContract =
                  user?.practiceReferralIncomeByLevel ||
                  (userData as any[])?.[19];

                const d =
                  directArray && Array.isArray(directArray)
                    ? directArray[i] ?? BigInt(0)
                    : BigInt(0);
                const w =
                  winnerArray && Array.isArray(winnerArray)
                    ? winnerArray[i] ?? BigInt(0)
                    : BigInt(0);
                const pFromContract =
                  practiceArrayFromContract &&
                  Array.isArray(practiceArrayFromContract)
                ...
                const pTotal =
                  stats.bonus > BigInt(0) ? stats.bonus : pFromContract;
                const cash = d + w;
                const isBasicLevel = i < 3;
                const isLevelActivated = isBasicLevel
                  ? isBasicActivated
                  : isProActivated;

                const isL1 = i === 0;

                const rowBorder = isLevelActivated
                  ? "border-l-2 border-l-green-500/60"
                  : "border-l-2 border-l-red-500/60";
                const activationLabel = isBasicLevel
                  ? isBasicActivated
                    ? "✅ BASIC ACTIVE"
                    : "🔴 BASIC LOCKED — Deposit 10 USDT"
                  : isProActivated
                  ? "✅ PRO ACTIVE"
                  : "🔴 PRO LOCKED — Deposit 100 USDT cumulative";
                    ...
                    <td className="px-4 py-4">
                      <p
                        className={`text-sm font-mono italic ${
                          pTotal > 0 ? "text-yellow-500" : "text-gray-500"
                        }`}
                      >
                        {formatUnits(pTotal, 18)}{" "}
        ...
        <div className="mt-6 pt-6 border-t border-gray-800/50 flex flex-col gap-3 text-xs">
          <div className="flex items-center gap-3">
            <span className="shrink-0 bg-green-500/15 border border-green-500/40 text-green-400 px-2 py-0.5 rounded font-black tracking-tighter uppercase">
              ✅ Green
            </span>
            <span className="text-gray-400">
              Level is <strong className="text-green-400">ACTIVE</strong> — you
              are earning cash income from this level.
            </span>
          </div>
          <div className="flex items-center gap-3">
            <span className="shrink-0 bg-red-500/15 border border-red-500/40 text-red-400 px-2 py-0.5 rounded font-black tracking-tighter uppercase">
              🔴 Red
            </span>
            <span className="text-gray-400">
              Level is <strong className="text-red-400">LOCKED</strong> —
              deposit more USDT to unlock. Basic (L1–L3): 10 USDT · Pro
              (L4–L15): 100 USDT cumulative.
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="bg-gray-900/40 p-6 md:p-8 rounded-3xl border border-gray-800 backdrop-blur-xl mb-8">
        ...
        {isLoadingHistory ? (
          <div className="py-10 text-center animate-pulse text-gray-500 italic text-sm">
            ...
            <table className="w-full text-left border-separate border-spacing-y-2">
              ...
              <tbody>
                {conversionHistory.map((item, i) => (
                  <tr
                    key={i}
                    className="bg-black/20 hover:bg-white/5 transition-all group rounded-xl"
                  >
      ...
      <div className="bg-surface/50 p-8 rounded-2xl border border-gray-800 min-h-[400px]">
        ...
        {isLoadingReferrals ? (
          ...
          <div className="text-center py-20 text-gray-500 italic">
            No partners found. Start inviting!
          </div>
        ) : (
          <div className="space-y-1">
            {directReferrals.map((ref, idx) => (
              <ReferralTreeItem
                ...
                address={ref}
                level={0}
                onShowDetails={(addr, lvl) =>
                  setSelectedUser({ address: addr, level: lvl })
                }
              />
            ))}
          </div>
      ...
      <div className="mt-8 grid md:grid-cols-2 gap-6">
        <div className="bg-gray-900/30 border border-gray-800 p-6 rounded-2xl">
          ...
          <div className="grid grid-cols-2 text-sm gap-2">
            ...
            <p className="text-right text-white font-bold">5%</p>
            <p className="text-gray-500">Level 2</p>
            <p className="text-right text-white font-bold">2%</p>
            <p className="text-gray-500">Level 3-5</p>
            <p className="text-right text-white font-bold">1%</p>
            <p className="text-gray-500">Level 6-15</p>
            <p className="text-right text-white font-bold">0.5%</p>
          </div>
        </div>
        <div className="bg-gray-900/30 border border-gray-800 p-6 rounded-2xl flex flex-col justify-center">
          <p className="text-sm text-gray-400 leading-relaxed italic">
            * Commissions are calculated based on partner deposits. Income is
            instantly credited to your available balance once eligibility
            criteria are met.
          </p>
        </div>
      </div>
