...
export default function IncomeHistory() {
  ...
  const isRegistered =
    (userObj as any).isRegistered ?? (userData as any[])?.[28] ?? false;
  const formatBal = (val: any) =>
    val ? Number(formatUnits(val, 18)).toFixed(2) : "0.00";

  const totalDeposit =
    (userObj as any).totalDeposit ?? (userData as any[])?.[6] ?? BigInt(0);
  ...
  const directInc =
    dList?.directReferralIncome ?? (userData as any[])?.[9] ?? BigInt(0);
  const winnerInc =
    dList?.winnerReferralIncome ?? (userData as any[])?.[10] ?? BigInt(0);
  const cashbackI =
    dList?.cashbackIncome ?? (userData as any[])?.[12] ?? BigInt(0);
  const lossRefI =
    dList?.lossReferralIncome ?? (userData as any[])?.[13] ?? BigInt(0);
  const clubI = dList?.clubIncome ?? (userData as any[])?.[14] ?? BigInt(0);

  const totalPracticeBalanceVal = dList?.practiceBalance ?? BigInt(0);

  const directIncomeVal =
    dList?.directReferralIncome ?? (userData as any[])?.[9] ?? BigInt(0);
  const winnerIncomeVal =
    dList?.winnerReferralIncome ?? (userData as any[])?.[10] ?? BigInt(0);
  const practiceRevVal =
    dList?.practiceReferralIncome ?? (userData as any[])?.[11] ?? BigInt(0);
  const cashbackIncVal =
    dList?.cashbackIncome ?? (userData as any[])?.[12] ?? BigInt(0);
  const lossRefIncVal =
    dList?.lossReferralIncome ?? (userData as any[])?.[13] ?? BigInt(0);
  const clubIncVal =
    dList?.clubIncome ?? (userData as any[])?.[14] ?? BigInt(0);
  ...
  const StatusBadge = ({ active }: { active: boolean }) => (
    <span
      className={`ml-3 px-2 py-0.5 rounded text-[10px] font-black tracking-tighter uppercase ${
        active
    ...
    const fetchIncomeLogs = async () => {
      try {
        ...
        if (data.success) {
          ...
          combined = [
            ...
            ...data.incomes
              .map((it: any) => {
                ...
                if (
                  srcLower.includes("practice") &&
                  srcLower.includes("referral")
                )
                  pn += amt;
                else if (
                  srcLower.includes("practice") &&
          ...
          convTotal =
            data.conversions?.reduce(
              (acc: bigint, it: any) => acc + BigInt(it.amount),
              BigInt(0),
            ) || BigInt(0);
        } else {
        ...
        const totalTeamEarningsVal =
          pt + pn > BigInt(0) ? pt + pn : practiceRevVal;
  ...
  if (!isConnected)
    return (
      <div className="text-center py-8 text-gray-400">
        Connect wallet to view income history
      </div>
    );
  if (loading)
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-bold text-primary italic uppercase tracking-tighter">
            � Revenue Overview
          </h2>
          <p className="text-[10px] font-black text-gray-500 uppercase tracking-widest mt-1">
            7 Cash Sources + Unlimited Practice Rewards
          </p>
        </div>
      ...
      <div className="bg-gradient-to-r from-green-600/20 to-green-900/20 border border-green-500/30 p-6 rounded-3xl mb-8">
        <p className="text-[10px] font-black text-green-400 uppercase tracking-[0.2em] mb-1">
          Total Web-Earned Available Balance
        </p>
        <h2 className="text-4xl font-black text-white italic uppercase tracking-tighter">
          ${formatBal(totalRealIncomeVal)}{" "}
          <span className="text-sm not-italic opacity-50 font-bold">USDT</span>
        </h2>
        <p className="text-[9px] text-gray-500 font-bold mt-2 uppercase">
          Includes 2X Game Wins + All Cash Referral & Pool Bonuses
        </p>
      </div>

      <div className="space-y-6">
        <h3 className="text-lg font-black text-yellow-500 flex items-center gap-2 italic uppercase tracking-tighter">
          <span className="bg-yellow-500/10 px-2 py-1 rounded-lg">💰</span> Real
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          >
            ...
            <p className="text-[9px] text-gray-500 font-bold uppercase mt-3 italic tracking-tight">
              Earn when direct team deposits
            </p>
          </div>
      ...
      <div className="bg-gradient-to-r from-blue-600/20 to-blue-900/20 border border-blue-500/30 p-6 rounded-3xl mb-8 relative overflow-hidden">
        ...
        <p className="text-[10px] font-black text-blue-400 uppercase tracking-[0.2em] mb-1">
          Total Practice Portfolio
        </p>
        <div className="flex items-baseline gap-3">
          <h2 className="text-4xl font-black text-white italic uppercase tracking-tighter">
            ${formatBal(totalPracticeBalanceVal)}
          </h2>
          <span className="text-sm not-italic opacity-50 font-bold text-blue-300">
            USDT
          </span>
        </div>
        <div className="mt-4 flex flex-wrap gap-4 text-[9px] text-gray-400 font-bold uppercase tracking-tight">
          ...
          <div className="bg-green-500/10 px-2 py-1 rounded border border-green-500/20 text-green-400">
            Total Earnings (Wins + Team): $
            {formatBal(
              BigInt(Math.floor(Number(pWinTotal.replace(/,/g, "")) * 1e18)) +
                BigInt(Math.floor(Number(pTeamTotal.replace(/,/g, "")) * 1e18)),
            )}
          </div>
        </div>
        <p className="text-[8px] text-gray-500 font-bold mt-4 uppercase tracking-tighter opacity-60">
          Demo balance for education and referral tier activation
        </p>
      </div>

      <div className="space-y-6 pt-4">
        <h3 className="text-lg font-black text-blue-400 flex items-center gap-2 italic uppercase tracking-tighter">
          <span className="bg-blue-500/10 px-2 py-1 rounded-lg">🎮</span>{" "}
          Practice Income Metrics
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="p-5 rounded-3xl border bg-blue-500/5 border-blue-500/20 shadow-[0_0_15px_rgba(59,130,246,0.1)]">
            ...
            <p className="text-[9px] text-gray-500 font-bold uppercase mt-3 italic tracking-tight">
              Earned from direct team registrations
            </p>
          </div>
          ...
          <div className="p-5 rounded-3xl border bg-orange-500/5 border-orange-500/20 shadow-[0_0_15px_rgba(249,115,22,0.1)]">
            ...
            <p className="text-[9px] text-gray-500 font-bold uppercase mt-3 italic tracking-tight">
              Practice converted to real balance after deposit
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-12 bg-[#0A0A0A] rounded-[2rem] border border-white/5 overflow-hidden shadow-2xl">
        <div className="p-8 border-b border-white/5">
          ...
          <p className="text-[10px] font-bold text-gray-500 uppercase tracking-widest mt-1">
            Full breakdown of all earnings and rewards.
          </p>
        </div>
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-8 py-6">
                        <span
                          className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-tighter ${
                            log.source === "Registration Bonus"
                              ? "bg-purple-500/10 text-purple-400"
                              : log.source.includes("Winner")
                              ? "bg-green-500/10 text-green-400"
                              : log.source.includes("Direct")
                              ? "bg-yellow-500/10 text-yellow-400"
                              : log.source.includes("Practice") ||
                                log.source.includes("Demo")
                              ? "bg-blue-500/10 text-blue-400"
                              : log.source.includes("Cashback")
                              ? "bg-blue-500/10 text-blue-400"
                              : log.source.includes("Lucky")
                        ...
                        >
                          {log.source === "Registration Bonus"
                            ? "🎁 Registration Bonus"
                            : log.source === "Cash Game Win"
                            ? "🎰 Cash Game Win"
                            : log.source === "Winner Referral"
                            ? "🤝 Team Win Bonus"
                            : log.source === "Direct Referral"
                            ? "👥 Direct Deposit ROI"
                            : log.source === "Practice Game Referral"
                            ? "🎮 Demo Team Bonus"
                            : log.source === "Loss Referral"
                            ? "🛡️ Team Cashback ROI"
                            : log.source.includes("Daily") ||
                              log.source === "Daily Cashback"
                            ? "📅 Daily Cashback"
                            : log.source === "Club Income"
                            ? "🏢 Club Pool Share"
                            : log.source.includes("Lucky")
                            ? "🎟️ Lucky Draw Prize"
                            : log.source}
                      ...
                      <td className="px-8 py-6 text-right">
                        ...
                        <span className="text-[10px] text-gray-600 ml-1 font-bold uppercase">
                          USDT
                        </span>
                      </td>
                    </tr>
