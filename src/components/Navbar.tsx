...
export default function Navbar() {
    ...
    const navItems = [
        ...
        { name: 'Analytics', path: '/admin/analytics', protected: true, adminOnly: true },
        { name: 'Distributions', path: '/admin/distributions', protected: true, adminOnly: true },
        ...
        <header className="border-b border-gray-800 bg-black/90 backdrop-blur-md sticky top-0 z-50">
            <div className="container mx-auto px-4 h-16 md:h-20 flex items-center justify-between">
                ...
                <Link href="/" className="flex items-center gap-2 shrink-0">
                    <img src="/logo.png" alt="Logo" className="w-12 h-12 md:w-14 md:h-14 rounded-lg" />
                </Link>
                    ...
                    {visibleNavItems.map((item) => {
                        ...
                        return (
                            <Link
                                key={item.path}
                                href={item.path}
                                className={`px-3 py-2 rounded-lg text-sm font-medium transition-colors whitespace-nowrap ${isActive
