...
function WinnerBalancesCard({ digitBalances }: { digitBalances: bigint[] }) {
  ...
  return (
    <motion.div variants={itemVar} className="col-span-1 md:col-span-2 lg:col-span-4 bg-yellow-500/5 border border-yellow-500/20 rounded-2xl p-6 backdrop-blur-sm">
      <div className="flex items-center gap-3 mb-4">
        <span className="text-xl">🏆</span>
        <div>
          <h3 className="text-yellow-400 font-black uppercase text-sm tracking-tighter">Winner Digit Balances (6X Re-investment)</h3>
          <p className="text-[10px] text-gray-400 italic">Winnings can only be re-invested on the digit they were won on.</p>
        </div>
      </div>
      <div className="grid grid-cols-5 md:grid-cols-10 gap-3">
        {digitBalances.map((bal, idx) => (
          <div key={idx} className={cn(
            ...
            bal > BigInt(0) ? "bg-yellow-500/20 border-yellow-500/50" : "bg-black/20 border-white/5 opacity-40"
          )}>
...
export default function Dashboard() {
  ...
  const [liveActiveDirects, setLiveActiveDirects] = useState<number | null>(null);

  useEffect(() => { setMounted(true); }, []);

  const isWrongNetwork = isConnected && chain?.id !== 56 && chain?.id !== 97 && chain?.id !== 31337;
  ...
  useEffect(() => {
    ...
    const minActivationFromSettings = (() => {
      ...
      if (s && typeof s === 'object') return s.minActivation ?? parseUnits('10', 18);
    ...
    (async () => {
      try {
        const directContracts = Array.from({ length: directReferralCount }, (_, i) => ({
          address: TRK_ADDRESSES.ROUTER,
          abi: TRKRouterABI.abi,
        ...
        const directAddresses = directResults
          .filter((r: any) => r?.status === 'success' && typeof r.result === 'string')
        ...
        for (const r of infoResults as any[]) {
          ...
          const isCashPlayer = Boolean(info?.isCashPlayer ?? info?.[30] ?? false);
          ...
          if (isCashPlayer || cumulativeBI >= MIN_ACTIVE_DEPOSIT || totalBI >= MIN_ACTIVE_DEPOSIT) active += 1;
        }
  ...
  }, [isConnected, address, publicClient, isWrongNetwork, directReferralCount, allSettings]);

  // --- LOADING / ERROR STATES ---
  if (!mounted) return <LoadingScreen />;
  if (!isConnected) return <EmptyState icon="🔌" title="Wallet Disconnected" desc="Please connect your wallet to access the dashboard." />;
  if (isWrongNetwork) return <NetworkErrorState chainName={chain?.name} chainId={chain?.id} />;
  ...
  const isActuallyRegistered = dataList?.isRegistered ?? dataList?.[28] ?? false;

  if (!isActuallyRegistered || !userId || BigInt(userId) === BigInt(0)) {
    return (
      <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="container mx-auto px-4 py-12 flex justify-center">
        <RegistrationPanel />
      </motion.div>
    );
  }
  ...
  const directReferralIncome = dataList?.directReferralIncome ?? dataList?.[9] ?? BigInt(0);
  const practiceBalance = dataList?.practiceBalance ?? dataList?.[4] ?? BigInt(0);
  ...
  const luckyDrawWallet = dataList?.luckyDrawWallet ?? dataList?.[16] ?? BigInt(0);
  const digitBalances = dataList?.digitBalances ?? dataList?.[20] ?? Array(10).fill(BigInt(0));
  const directReferrals = dataList?.directReferrals ?? dataList?.[31] ?? BigInt(0);
  ...
  const activeDirectsDisplay = Math.max(Number(activeDirects ?? BigInt(0)), liveActiveDirects ?? 0);
  
  // Literal Gross Income (All sources)
  const totalEarningsGross = (
      (dataList?.directReferralIncome ?? (Array.isArray(dataList) ? dataList[9] : BigInt(0))) +
      (dataList?.winnerReferralIncome ?? (Array.isArray(dataList) ? dataList[10] : BigInt(0))) +
      (dataList?.cashbackIncome ?? (Array.isArray(dataList) ? dataList[12] : BigInt(0))) +
      (dataList?.lossReferralIncome ?? (Array.isArray(dataList) ? dataList[13] : BigInt(0))) +
      (dataList?.clubIncome ?? (Array.isArray(dataList) ? dataList[14] : BigInt(0))) +
      (dataList?.luckyDrawIncome ?? (Array.isArray(dataList) ? dataList[15] : BigInt(0))) +
      (dataList?.totalWins ?? (Array.isArray(dataList) ? dataList[23] : BigInt(0)))
  );
  ...
  const formatBal = (val: any) => val ? Number(formatUnits(val, 18)).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '0.00';
  
  const displayUsdtBalance = usdtBalance ? formatUnits(usdtBalance as bigint, 18) : '0';
        ...
        <AnimatePresence mode='popLayout'>
          {hasLowUsdt && (
            <AlertBanner 
              ...
              desc={`Wallet: ${Number(displayUsdtBalance).toFixed(2)} USDT. You need USDT to play.`}
              action={{ label: "Get USDT", url: "https://pancakeswap.finance/swap?outputCurrency=0x55d398326f99059fF775485246999027B3197955" }}
            />
          )}
          {isPractice && !isCash && (
      ...
      <div className="space-y-12">
        ...
        <section className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                <StatCard title="Practice Balance" value={`$${formatBal(practiceBalance)}`} sub="Account Always Active" icon="🎯" highlight />
        ...
        <section className="space-y-6">
            <h2 className="text-2xl font-black text-yellow-500 flex items-center gap-3 italic uppercase tracking-tighter">
                <span className="bg-yellow-500/20 p-2 rounded-xl not-italic">💰</span> 
                Real Cash Arena
            </h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <StatCard title="Cash Game Balance" value={`$${formatBal(cashGameBalance)}`} sub="Available to Play" icon="🔥" color="text-yellow-400" highlight />
                <StatCard title="Wallet Balance" value={`$${formatBal(walletBalance)}`} sub="Withdrawable" icon="💼" />
                <StatCard title="Total Deposit" value={`$${formatBal(totalDeposit)}`} icon="📥" />
                <StatCard title="Lucky Reward Wallet" value={`$${formatBal(luckyDrawWallet)}`} sub="For Auto-Tickets" icon="🎟️" color="text-pink-400" />
                <CashVerificationCard isCash={isCash} deposit={totalDeposit} regTime={dataList?.[2]} />
                <StatCard title="Total Referrals" value={totalReferralsDisplay.toString()} sub={`${activeDirectsDisplay} Active`} icon="👥" />
                <BoosterStatusCard 
            ...
            {!isCash && (
                <div className="space-y-4">
                    <motion.div variants={itemVar} className="w-full flex flex-col items-center justify-center py-4 bg-yellow-500/5 border border-dashed border-yellow-500/30 rounded-2xl p-6">
                        <div className="text-center mb-4">
                            <h4 className="text-yellow-500 font-black uppercase text-sm italic tracking-tighter">Bridge to Cash (Conversion Model)</h4>
                            <p className="text-[10px] text-gray-500 max-w-md mt-1 italic">To convert Practice rewards into real-value assets: Direct Referral Income ≥ {allSettings ? formatUnits((allSettings as any)[8] || parseUnits('100', 18), 18) : '100'} USDT & Deposit ≥ {allSettings ? formatUnits((allSettings as any)[8] || parseUnits('100', 18), 18) : '100'} USDT requirement applies.</p>
                        </div>
        ...
        <motion.div variants={itemVar} className="lg:col-span-1">
          <div className="sticky top-8 space-y-6">
             ...
             <div className="bg-white/5 border border-white/10 p-6 rounded-3xl backdrop-blur-md">
                <h4 className="text-gray-400 font-bold text-sm uppercase mb-2">My Transactions</h4>
             ...
             <div className="bg-blue-900/10 border border-blue-500/20 p-6 rounded-3xl backdrop-blur-md">
                <h4 className="text-blue-400 font-bold text-sm uppercase mb-2">Need Help?</h4>
                <p className="text-xs text-blue-200/70 leading-relaxed">
                    If you have issues with deposits or withdrawals, please contact support or check the community guides.
                </p>
             </div>
          </div>
        </motion.div>
...
function StatCard({ title, value, sub, icon, color = "text-white", highlight }: any) {
  return (
    <motion.div 
      variants={itemVar}
      ...
      className={cn(
        ...
        highlight ? "bg-blue-500/10 border-blue-500/50 shadow-[0_0_15px_rgba(59,130,246,0.1)]" : "bg-white/5 border-white/10 hover:bg-white/10"
      )}
    >
      ...
      <div className={cn("text-2xl md:text-3xl font-bold font-mono tracking-tight break-all", color)}>
        {value}
      </div>
...
function PracticeVerificationCard() {
  return (
    <motion.div variants={itemVar} className="p-6 rounded-2xl border border-blue-500/30 bg-blue-500/10 flex flex-col justify-center items-center text-center backdrop-blur-md">
      <span className="text-[10px] font-black uppercase tracking-[0.2em] text-blue-400/60 mb-1">Practice Account</span>
      <span className="text-xl font-black text-blue-400 italic tracking-tighter uppercase">Verified</span>
      <div className="flex items-center gap-1.5 mt-2 px-3 py-1 bg-blue-500/20 rounded-full border border-blue-500/30">
        ...
        <span className="text-[8px] font-black text-blue-300 uppercase">Live Practice ID</span>
      </div>
    </motion.div>
  );
}

function CashVerificationCard({ isCash, deposit, regTime }: { isCash: boolean, deposit?: bigint, regTime?: bigint }) {
  const depAmt = deposit ? Number(formatUnits(deposit, 18)) : 0;
  const isBasic = depAmt >= 10;
  const isPro = depAmt >= 100;
  
  const status = isPro ? 'PRO ACTIVATED' : isBasic ? 'BASIC ACTIVATED' : isCash ? 'LOW BALANCE' : 'UNVERIFIED';
  const color = isPro ? 'border-yellow-500/50 bg-yellow-500/20 shadow-yellow-500/10' : 
                isBasic ? 'border-blue-500/30 bg-blue-500/10' : 
                'border-red-500/20 bg-red-500/5';
  const textColor = isPro ? 'text-yellow-400 font-black' : isBasic ? 'text-blue-400' : 'text-red-400';

  const regTimestamp = regTime ? Number(regTime) : 0;
  const daysSinceReg = regTimestamp > 0 ? (Math.floor(Date.now() / 1000) - regTimestamp) / 86400 : 0;
  const needsAction = !isBasic && daysSinceReg > 20; // 20 days warn
  
  return (
    <motion.div variants={itemVar} className={cn("p-6 rounded-2xl border flex flex-col justify-center items-center text-center backdrop-blur-md relative overflow-hidden", color)}>
      <span className="text-[10px] font-black uppercase tracking-[0.2em] opacity-40 mb-1">Ecosystem Status</span>
      <span className={cn("text-xl font-black italic tracking-tighter uppercase", textColor)}>{status}</span>
      
      <div className="flex flex-col gap-1 mt-2">
           <span className={cn("text-[8px] font-black uppercase", isBasic ? 'text-green-400' : 'text-gray-500')}>• L1-L3 Income {isBasic ? '✅' : '🔒'}</span>
           <span className={cn("text-[8px] font-black uppercase", isPro ? 'text-green-400' : 'text-gray-500')}>• L4-L15 Income {isPro ? '✅' : '🔒'}</span>
      </div>
...
function BoosterStatusCard({ 
  ...
  const sortedCaps = [...filteredCaps].sort((a,b) => Number(b.directs) - Number(a.directs));
  ...
  const progress = maxCap > 0 ? Math.min((currentEarned / maxCap) * 100, 100) : 0;
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start mb-4">
          <div>
              <span className="text-[10px] font-black uppercase tracking-[0.2em] text-purple-400 opacity-80">Income Booster Status</span>
              <div className="flex items-center gap-2 mt-1">
                  <h3 className="text-3xl font-black text-white italic tracking-tighter uppercase">{multiplier}X LIMIT</h3>
                  <span className="bg-purple-600 text-[8px] font-black px-2 py-0.5 rounded text-white">{multiplier * 100}% ROI CAP</span>
              </div>
          </div>
          <div className="text-right">
              <span className="text-[10px] font-bold text-gray-500 uppercase block leading-none">Phase</span>
              <span className="text-xs font-black text-purple-400">{isPostThreshold ? 'Global Beta 10K+' : 'Seed Launch Area'}</span>
          </div>
      </div>
      ...
      <div className="space-y-4">
          ...
          <div className="flex justify-between items-end">
              <div className="space-y-1">
                  <span className="text-[9px] font-black text-gray-500 uppercase tracking-widest block">Earned vs Cap</span>
                  <div className="flex items-baseline gap-1.5">
                      <span className="text-lg font-black text-white">${currentEarned.toFixed(2)}</span>
                      <span className="text-[10px] text-gray-600 font-bold uppercase">/ ${maxCap.toFixed(2)} USDT</span>
                  </div>
              </div>
 
              {nextTier ? (
                  <div className="text-right bg-black/40 px-3 py-2 rounded-xl border border-white/5">
                      <span className="text-[8px] font-black text-purple-400/60 uppercase block mb-0.5">Next Boost at {nextTier.directs} Directs</span>
                      <span className="text-xs font-black text-white italic tracking-tighter">Increase to {Number(nextTier.multiplier)}X Limit</span>
                  </div>
              ) : (
                  <div className="text-right">
                      <span className="text-[10px] font-black text-gray-500 uppercase">Maximum Boost Achieved</span>
                  </div>
              )}
          </div>
          
          <p className="text-[8px] text-gray-500 italic opacity-60">
              *Income Cap depends on your active directs (Deposit ≥ 100 USDT). Current multiplier: {multiplier}X.
          </p>
      </div>
    </motion.div>
  );
...
function CashbackStatusCard({ isNetProfit }: { isNetProfit: boolean }) {
  ...
  const color = isActive ? 'text-green-400 border-green-500/30 bg-green-500/10' : 'text-red-400 border-red-500/30 bg-red-500/10';
  const desc = isActive ? 'Receiving 0.5% Cashback on Net Losses' : 'Profit > Loss (No Cashback)';
  
  return (
    <motion.div variants={itemVar} className={cn("p-6 rounded-2xl border flex flex-col justify-center items-center text-center backdrop-blur-md", color, "col-span-1 md:col-span-2 shadow-lg")}>
      <span className="text-xs font-bold uppercase tracking-widest opacity-80 mb-1">0.5% Loss Cashback</span>
      <span className="text-3xl font-black tracking-wider">{status}</span>
      <span className="text-[10px] uppercase font-bold mt-2 opacity-80">{desc}</span>
    </motion.div>
  );
}

function ReferralCodeCard({ code, isHero }: { code: string, isHero?: boolean }) {
  ...
  return (
    <motion.div 
      ...
      className={cn(
        ...
        isHero ? "p-4 py-6 md:py-8 border-dashed shadow-2xl shadow-purple-500/10" : "p-6"
      )}
    ...
    >
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        <div className="mt-2 flex items-center gap-2">
            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest group-hover:text-purple-400 transition-colors">Click to Copy</span>
...
function AlertBanner({ type, title, desc, action }: any) {
  ...
  return (
    <motion.div 
      ...
      className={cn("border rounded-xl p-4 flex flex-col sm:flex-row items-center justify-between gap-4 backdrop-blur-md", colors)}
    >
      <div className="flex items-center gap-3">
        <span className="text-2xl animate-bounce">{type === 'error' ? '⚠️' : 'ℹ️'}</span>
        <div>
          <h4 className={cn("font-bold text-sm", type === 'error' ? 'text-red-400' : 'text-blue-400')}>{title}</h4>
      ...
      {action && (
        <a 
          ...
          target="_blank" 
          rel="noreferrer" 
          className={cn("px-4 py-2 rounded-lg text-xs font-bold uppercase transition-all whitespace-nowrap hover:scale-105 active:scale-95 shadow-lg", type === 'error' ? 'bg-red-600 hover:bg-red-500 text-white shadow-red-500/20' : 'bg-blue-600 hover:bg-blue-500 text-white')}
        >
          {action.label}
        </a>
      )}
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-4 relative z-10">
        ...
        <div>
          <h4 className="text-blue-400 font-bold text-sm">Practice Mode Active</h4>
          <p className="text-xs text-blue-200/80">Balance: <span className="font-mono text-white">{balance} USDT</span></p>
        </div>
      </div>
    ...
    >
      <div className="flex flex-col md:flex-row items-center justify-between gap-4">
        <div className="text-center md:text-left">
            <p className="text-red-400 text-xs font-black uppercase mb-1 tracking-[0.2em] animate-pulse">⚠️ CRITICAL: 30-Day Activation Rule</p>
            <p className="text-[10px] text-gray-400 max-w-xs">Upgrade to Real Cash within 30 days or your <span className="text-white font-bold">ID and Practice Balance will be permanently deleted</span> from the blockchain.</p>
        </div>
...
const TimeBox = ({ val, label }: { val: number, label: string }) => (
  <div className="flex flex-col items-center">
    ...
    <span className="text-[10px] text-orange-500/70 mt-1 font-bold">{label}</span>
  </div>
);
...
function ActivateRealCashButton() {
  ...
  const minActivationRaw = systemSettings ? (systemSettings as any)[0]?.[0] : null;
  const requiredUsdt = minActivationRaw ? BigInt(minActivationRaw) : parseUnits('10', 18);
  const displayUsdtAmount = Number(formatUnits(requiredUsdt, 18)).toString();

  const userUsdtBalance = usdtBalance ? BigInt(usdtBalance as bigint) : BigInt(0);
  ...
  const { isLoading: isActivateConfirming, isSuccess: isActivateConfirmed } = useWaitForTransactionReceipt({ 
    hash: activateHash 
  });
  ...
  useEffect(() => {
      ...
      const syncActivationDeposit = async () => {
        ...
        try {
          ...
          if (publicClient) {
            const receipt = await publicClient.getTransactionReceipt({ hash: activateHash });
            const incomeEvent = parseAbiItem('event IncomeReceived(address indexed user, uint256 amount, string source, string walletType, uint256 timestamp)');
            receipt.logs.forEach((log: any) => {
              try {
                const decoded = decodeEventLog({ abi: [incomeEvent], data: log.data, topics: log.topics });
  ...
  const isProcessing = isApproveSigning || isApproveConfirming || isActivateSigning || isActivateConfirming;
      ...
      <div className="w-full bg-yellow-500/5 border border-yellow-500/20 p-6 rounded-2xl space-y-4 backdrop-blur-sm">
          {!hasEnoughUsdt && (
              <div className="p-3 bg-red-500/10 border border-red-500/30 rounded-lg text-center font-mono">
                  <p className="text-red-400 font-bold text-xs">⚠️ Need {displayUsdtAmount} USDT to Activate</p>
              </div>
          )}
          
          <div className="flex flex-col md:flex-row items-center justify-between gap-4">
              <div>
                  <h4 className="text-yellow-400 font-bold text-sm uppercase">Upgrade Account</h4>
                  <p className="text-xs text-gray-400">Upgrade to Real Cash ({displayUsdtAmount} USDT)</p>
              </div>
              ...
              {needsApproval ? (
                ...
                >
                    {isApproveSigning ? '⏳ Signing...' : isApproveConfirming ? '⏳ Approving...' : '1. Approve USDT'}
                </button>
              ) : (
                <button 
                ...
                >
                    {isActivateSigning ? '⏳ Signing...' : isActivateConfirming ? '⏳ Activating...' : '2. Activate Now'}
                </button>
              )}
          </div>
...
function DepositSection() {
  ...
  const { data: usdtBalance } = useReadContract({ address: TRK_ADDRESSES.USDT, abi: USDTABI.abi, functionName: 'balanceOf', args: [address], query: { refetchInterval: 30000 } });
  const { data: allowance, refetch: refetchAllowance } = useReadContract({ address: TRK_ADDRESSES.USDT, abi: USDTABI.abi, functionName: 'allowance', args: [address, TRK_ADDRESSES.TREASURY], query: { refetchInterval: 30000 } });
  ...
  const dataList = userData ? (userData as any) : null;
  const isRegistered = dataList?.isRegistered ?? ((dataList as any[])?.[28] || false);
  const isCashPlayer = dataList?.isCashPlayer ?? ((dataList as any[])?.[30] || false);
  ...
  const { writeContract: writeApprove, data: approveHash, isPending: isApproveSigning } = useWriteContract();
  const { isLoading: isApproveConfirming, isSuccess: isApproveConfirmed } = useWaitForTransactionReceipt({ hash: approveHash });
  const { writeContract: writeDeposit, data: depositHash, isPending: isDepositSigning } = useWriteContract();
  const { isLoading: isDepositConfirming, isSuccess: isDepositConfirmed } = useWaitForTransactionReceipt({ hash: depositHash });

  useEffect(() => { if (isApproveConfirmed) { setStep('deposit'); refetchAllowance(); } }, [isApproveConfirmed, refetchAllowance]);
  useEffect(() => { 
    if (isDepositConfirmed && depositHash && address) {
        ...
        const syncDeposit = async () => {
            try {
                ...
                const now = Math.floor(Date.now() / 1000);
                const conversionEvent = parseAbiItem('event PracticeConverted(address indexed user, uint256 amount, uint256 timestamp)');
                const incomeEvent = parseAbiItem('event IncomeReceived(address indexed user, uint256 amount, string source, string walletType, uint256 timestamp)');

                receipt.logs.forEach((log: any) => {
                  try {
                    const decoded = decodeEventLog({ abi: [conversionEvent], data: log.data, topics: log.topics });
                    if (decoded.eventName === 'PracticeConverted' && (decoded.args as any).user.toLowerCase() === address.toLowerCase()) {
  ...
  const handleDeposit = () => {
      ...
      if (!amount || amount < minLimit) return setError(`❌ Min ${minLimit} USDT`);
      ...
      if (currentAllow < amountWei) {
          setStep('approve');
          writeApprove({ address: TRK_ADDRESSES.USDT, abi: USDTABI.abi, functionName: 'approve', args: [spender, amountWei] });
      } else {
          setStep('deposit');
          writeDeposit({ address: TRK_ADDRESSES.ROUTER, abi: TRKRouterABI.abi, functionName: 'depositCashGame', args: [amountWei] });
      }
  };

  const userBalanceFormatted = usdtBalance ? formatUnits(usdtBalance as bigint, 18) : '0';
  ...
  return (
      <div className="bg-gradient-to-b from-blue-900/20 to-purple-900/20 border border-white/10 rounded-2xl p-6 backdrop-blur-xl shadow-xl">
          ...
          {error && <div className="mb-4 p-3 bg-red-500/20 border border-red-500/50 rounded-lg text-red-200 text-xs font-bold">{error}</div>}
          ...
          <div className="mb-4 space-y-2">
              ...
              <div className="relative">
                  <input
                      ...
                      disabled={!canDeposit}
                  />
                  <span className="absolute right-4 top-3 text-sm text-gray-500 font-bold">USDT</span>
              </div>
              <div className="grid grid-cols-2 gap-2 mt-3">
                  <div className="p-2 bg-blue-500/5 border border-blue-500/20 rounded-lg text-center">
                      <p className="text-[9px] font-black text-blue-400 uppercase">Basic (10$)</p>
                      <p className="text-[7px] text-gray-500 uppercase mt-0.5">Unlock 3 Levels</p>
                  </div>
                  <div className="p-2 bg-yellow-500/5 border border-yellow-500/20 rounded-lg text-center">
                      <p className="text-[9px] font-black text-yellow-500 uppercase">Pro (100$)</p>
                      <p className="text-[7px] text-gray-500 uppercase mt-0.5">Unlock All 15</p>
                  </div>
              </div>
          </div>
          ...
          <button
              ...
              disabled={!depositAmount || !canDeposit || isApproveSigning || isApproveConfirming || isDepositSigning || isDepositConfirming}
          ...
          >
              {!isRegistered ? 'Register First' : !isActive ? 'Activate Account' : 
               isApproveSigning ? 'Approving...' : isApproveConfirming ? 'Confirming...' :
               isDepositSigning ? 'Depositing...' : isDepositConfirming ? 'Confirming...' : 'Deposit Now'}
          </button>
          
          <div className="mt-4 text-center">
              <p className="text-[10px] text-gray-500 uppercase tracking-widest">Secured by BSC Smart Contract</p>
          </div>
      </div>
  );
}
...
function WithdrawSection() {
            ...
            try {
                await fetch(API_ENDPOINTS.SYNC_WITHDRAW, {
                    ...
                    body: JSON.stringify({
                        ...
                        amountSent: (parseUnits(amount, 18) * BigInt(9) / BigInt(10)).toString(),
                        timestamp: Math.floor(Date.now() / 1000)
                    })
                });
  ...
  const handleWithdraw = () => {
    ...
    if (val > parseFloat(formattedBalance)) return setError('Insufficient Wallet Balance');
      ...
      <div className="bg-gradient-to-b from-gray-900/40 to-black/40 border border-white/10 rounded-2xl p-6 backdrop-blur-xl shadow-xl mt-6">
          ...
          <div className="mb-4 space-y-2">
              <div className="flex justify-between text-xs text-gray-400">
                  ...
                  <span className="cursor-pointer hover:text-white" onClick={() => setAmount(formattedBalance)}>Max: {Number(formattedBalance).toFixed(2)}</span>
              </div>
              <div className="relative">
                  <input
                      type="number"
          ...
          >
              {isWithdrawPending ? 'Signing...' : isWithdrawConfirming ? 'Processing...' : 'Withdraw'}
          </button>
      </div>
  );
}
...
function TransactionHistory({ registrationTime }: { registrationTime?: bigint }) {
  const { address } = useAccount();
  const publicClient = usePublicClient();
      ...
      try {
        ...
        if (data.success) {
          const allCashLogs = [
            ...
            ...(data.incomes || []).filter((l:any) => l.walletType === 'Cash').map((l: any) => ({
              ...l,
          ...
          const allPracticeLogs = [
            ...
            ...(data.incomes || []).filter((l:any) => l.walletType === 'Practice').map((l: any) => ({
                ...l,
          ...
          const hasSignupLog = allPracticeLogs.some((l: any) => String(l.type || '').toLowerCase().includes('signup'));
  ...
  if(isLoading) return <div className="text-center text-gray-500 text-xs py-4">Loading history...</div>;
    ...
    <div className="mt-4">
      <div className="flex bg-black/40 p-1 rounded-xl mb-4 border border-white/5">
         <button 
           ...
           className={cn("flex-1 py-2 text-[10px] font-bold rounded-lg transition-all", !showPractice ? "bg-white/10 text-white" : "text-gray-500 hover:text-gray-300")}
         >
           CASH
         </button>
         <button 
           onClick={() => setShowPractice(true)}
           className={cn("flex-1 py-2 text-[10px] font-bold rounded-lg transition-all", showPractice ? "bg-blue-500/20 text-blue-400" : "text-gray-500 hover:text-gray-300")}
         >
           PRACTICE
         </button>
      </div>
      ...
      {activeLogs.length === 0 ? (
        ...
        <div className="space-y-3 max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
          {activeLogs.map((log: any, i) => (
            <div key={(log.hash || 'tx') + i} className="group flex justify-between items-center bg-white/5 p-4 rounded-xl border border-white/5 text-xs hover:bg-white/[0.08] transition-colors">
              <div className="flex items-center gap-3">
                 <span className={cn("w-10 h-10 flex items-center justify-center rounded-xl text-lg", 
                    log.type === 'Deposit' ? 'bg-green-500/10 text-green-400' : 
                    log.type === 'Withdraw' ? 'bg-red-500/10 text-red-400' : 
                    'bg-blue-500/10 text-blue-400'
                 )}>
                    {log.type === 'Deposit' ? '📥' : log.type === 'Withdraw' ? '📤' : '🎁'}
                 </span>
                 <div>
                   ...
                   <p className="text-[10px] text-gray-500 font-mono">
                     {log.timestamp ? new Date(Number(log.timestamp) * 1000).toLocaleDateString() : 'Recent'}
                   </p>
                 </div>
              </div>
               <div className="text-right">
                ...
                {log.type === 'Withdraw' && (
                  <p className="text-[8px] text-red-500 font-bold">
                    Net: {formatUnits(log.sent || BigInt(0), 18)} (Fee: {formatUnits(log.fee || BigInt(0), 18)})
                  </p>
                )}
                {log.type === 'Deposit' && (
                ...
                <p className="text-[9px] text-gray-600 font-bold uppercase">{log.type === 'Lucky Ticket' ? 'Tickets' : 'USDT'}</p>
              </div>
            </div>
          ))}
        </div>
...
function LoadingScreen() {
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-6">
      ...
      <p className="text-gray-500 text-xs font-mono uppercase tracking-[0.2em] animate-pulse">Initializing Ecosystem...</p>
    </div>
  );
}
...
function EmptyState({ icon, title, desc }: any) {
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex flex-col items-center justify-center min-h-[60vh] text-center p-8">
...
function NetworkErrorState({ chainName, chainId }: any) {
  return (
    <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} className="flex flex-col items-center justify-center min-h-[60vh] p-4">
      <div className="bg-red-900/20 border border-red-500/50 rounded-3xl p-10 max-w-md text-center backdrop-blur-xl shadow-2xl shadow-red-900/20">
        ...
        <p className="text-gray-300 mb-8 leading-relaxed">
          Connected to <span className="font-bold text-white">{chainName} ({chainId})</span>. 
          <br/>Please switch to <span className="font-bold text-yellow-400">BSC Mainnet (56)</span>.
        </p>
  ...
  return (
    <div className="w-full max-w-7xl mx-auto mt-8 px-4 space-y-8 animate-pulse">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="h-40 bg-gray-800/30 rounded-2xl border border-gray-800"></div>
        ))}
      </div>
