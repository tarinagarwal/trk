...
export default function LuckyDrawSection() {
  ...
  const luckyWallet = userDataRouter
    ? (userDataRouter as any).luckyDrawWallet ?? (userDataRouter as any[])?.[16]
    : BigInt(0);
  ...
  const goldenId = drawStats
    ? Number(drawStatsObj?.drawId ?? drawStatsObj?.[0] ?? 0)
    : 0;
  const silverId = silverStats
    ? Number(silverStatsObj?.drawId ?? silverStatsObj?.[0] ?? 0)
    : 0;
  const goldenTicketsCurrent = drawStats
    ? Number(drawStatsObj?.ticketsSold ?? drawStatsObj?.[1] ?? 0)
    : 0;
  const silverTicketsCurrent = silverStats
    ? Number(silverStatsObj?.ticketsSold ?? silverStatsObj?.[1] ?? 0)
    : 0;
  ...
  const [backendStats, setBackendStats] = useState({
    totalTicketsSilver: 0,
    totalTicketsGolden: 0,
    silverPool: "0",
    goldenPool: "0",
  });
  const {
  ...
  const { userGoldenTicketsCurrent, userSilverTicketsCurrent } = useMemo(() => {
    const counts = purchaseHistory.reduce(
      (acc, h) => {
        if (h.type === "GOLDEN" && h.drawId === goldenId)
          acc.golden += Number(h.count || 0);
        if (h.type === "SILVER" && h.drawId === silverId)
          acc.silver += Number(h.count || 0);
        return acc;
      },
      { golden: 0, silver: 0 },
    );
    ...
    return {
      userGoldenTicketsCurrent: counts.golden,
      userSilverTicketsCurrent: counts.silver,
    };
  }, [purchaseHistory, goldenId, silverId]);

  const currentPref = (userData as any)?.preferredLuckyDraw ?? 0;

  const maxTickets = config?.lucky.maxTickets
    ? Number(config.lucky.maxTickets)
    : 10000;
  const goldenPrice = config?.lucky.goldenPrice
    ? Number(formatUnits(config.lucky.goldenPrice, 18))
    : 10;
  const silverPrice = config?.lucky.silverPrice
    ? Number(formatUnits(config.lucky.silverPrice, 18))
    : 1;

  const pricePerTicket = selectedDraw === 1 ? goldenPrice : silverPrice; // 1 is GOLDEN
  const totalCostWei = parseUnits(
    (Number(ticketCount) * pricePerTicket).toString(),
    18,
  );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white/5 border border-white/10 rounded-3xl p-6 mb-2">
        <div>
          ...
          <p className="text-gray-500 text-xs">
            Your cashback & referral ROI automatically fuels your chosen draw.
          </p>
        </div>
        <div className="flex flex-col sm:flex-row items-center gap-4">
          ...
          <div className="bg-black/40 border border-white/5 px-6 py-2.5 rounded-2xl flex items-center gap-3 w-full sm:w-auto overflow-hidden">
            <div className="flex flex-col">
              <span className="text-[7px] font-black text-gray-500 uppercase tracking-widest leading-none mb-0.5">
                Lucky Reward Wallet
              </span>
              <div className="flex items-baseline gap-1">
                <span className="text-sm font-black text-pink-500">
                  {Number(formatUnits(luckyWallet, 18)).toFixed(2)}
                </span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <DrawCard
          ...
          ticketsSold={
            drawStats ? goldenTicketsCurrent : backendStats.totalTicketsGolden
          }
          userTickets={userGoldenTicketsCurrent}
          maxTickets={
            config?.lucky.maxTickets ? Number(config.lucky.maxTickets) : 10000
          }
        ...
        <DrawCard
          ...
          ticketsSold={
            silverStats ? silverTicketsCurrent : backendStats.totalTicketsSilver
          }
          ...
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-gray-500 text-sm mb-6">
                {selectedDraw === 1 ? "🥇 Golden Tier" : "🥈 Silver Tier"} • $
                {selectedDraw === 1 ? goldenPrice : silverPrice} per ticket
              </p>
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-between items-center py-2">
                  ...
                  <span className="text-xl font-black text-white">
                    ...
                    {(
                      Number(ticketCount) *
                      (selectedDraw === 1 ? goldenPrice : silverPrice)
                    ).toLocaleString()}{" "}
                    USDT
                  </span>
                </div>
                ...
                {/* Buy with Lucky Wallet */}
                {(() => {
                  const ticketPrice = parseUnits(
                    (selectedDraw === 1 ? goldenPrice : silverPrice).toString(),
                    18,
                  );
                  const totalCost = ticketPrice * BigInt(ticketCount || "1");
                  const hasLuckyBalance = luckyWallet >= totalCost;
                  return (
                    <button
                      onClick={() => {
                        if (!hasLuckyBalance) return;
                        setPendingTxType("BUY");
                        writeContract({
                          address: TRK_GAME_ADDRESS,
                          abi: TRKGameABI.abi,
                          functionName: "buyLuckyTicketsVirtual",
                          args: [BigInt(ticketCount), selectedDraw as 0 | 1],
                        });
                      }}
                      disabled={
                        isProcessing ||
                      ...
                      className="w-full py-3 rounded-2xl font-black text-sm transition-all active:scale-95 bg-pink-500/20 text-pink-400 border border-pink-500/30 hover:bg-pink-500/30 disabled:opacity-40"
                    >
                      {isProcessing
                        ? "PROCESSING..."
                        : !hasLuckyBalance
                        ? `NEED ${Number(
                            formatUnits(totalCost - luckyWallet, 18),
                          ).toFixed(2)} MORE IN LUCKY WALLET`
                        : `🎟️ USE LUCKY WALLET (${Number(
                            formatUnits(luckyWallet, 18),
                          ).toFixed(2)} USDT)`}
                    </button>
                  );
                })()}
      ...
      <div className="bg-blue-600/10 border border-blue-500/20 p-6 rounded-3xl mt-8">
        ...
        <p className="text-sm text-gray-400 leading-relaxed">
          When you buy a ticket, the USDT is immediately transferred to the{" "}
          <span className="text-white font-bold">Treasury</span>. From there, it
          is split between the <span className="text-white">Prize Pool</span>,
          the <span className="text-white">Protection Pool</span>, and{" "}
          <span className="text-white">Ecosystem Rewards</span> (Creator, BD,
          Ops) as per the smart contract distribution logic.
        </p>
      </div>
              ...
              <tbody>
                {purchaseHistory.map((h, i) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-4">
                      {(h.type === "GOLDEN" && h.drawId < goldenId) ||
                      (h.type === "SILVER" && h.drawId < silverId) ? (
            ...
            >
              ...
              <td className="px-4 py-4">
                ...
                >
                  {d.jackpotWinner === address
                    ? "YOU WON! 🏆"
                    : `${d.jackpotWinner.slice(0, 6)}...${d.jackpotWinner.slice(
                        -4,
                      )}`}
                </span>
              </td>
              <td className="px-4 py-4 text-right text-gray-600 font-mono text-[10px]">
                {d.block}
              </td>
            </tr>
          ))}
...
function DrawCard({
  ...
  return (
    <motion.div
      whileHover={{ y: -5 }}
      className={`relative overflow-hidden bg-gradient-to-br ${gradient} border ${
        isGolden ? "border-yellow-500/30" : "border-blue-500/30"
      } rounded-3xl p-6 shadow-2xl group transition-all`}
    >
      <div
        className={`absolute top-0 right-0 w-32 h-32 ${
          isGolden ? "bg-yellow-500/10" : "bg-blue-500/10"
        } rounded-full blur-[60px] -z-10 group-hover:scale-150 transition-transform duration-500`}
      ></div>
      ...
      <div className="flex justify-between items-start mb-6">
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            <span
              className={`text-[10px] ${
                isGolden ? "bg-yellow-500 text-black" : "bg-blue-500 text-white"
              } px-2 py-0.5 rounded-full uppercase font-black tracking-wider`}
            >
              {entryFee} USDT ENTRY
            </span>
            <span className="text-gray-500 text-[10px] font-bold">
              {maxT.toLocaleString()} TICKETS
            </span>
          </div>
        </div>
      ...
      <div className="mb-6">
        <p
          className={`text-[10px] ${
            isGolden ? "text-yellow-500" : "text-blue-400"
          } uppercase tracking-widest font-black mb-1 opacity-80`}
        >
          Current Jackpot
        </p>
        <div className="flex items-baseline gap-2">
          <div className="text-4xl font-black text-white tracking-tight leading-none">
            $
            {poolAmount.toLocaleString(undefined, {
      ...
      <div className="mb-6">
        <div className="flex justify-between items-end mb-2">
          ...
          >
            {soldT.toLocaleString()} / {maxT.toLocaleString()}
          </span>
        </div>
        <div className="flex justify-between items-center mb-2 text-[9px] uppercase tracking-widest">
          ...
          <span
            className={
              isGolden
                ? "text-yellow-500/80 font-black"
                : "text-blue-400/80 font-black"
            }
          >
      ...
      <button
        onClick={onBuy}
        className={`w-full py-3 rounded-xl font-black text-xs uppercase tracking-widest shadow-xl transition-all active:scale-95 ${
          isGolden
      ...
      <p className="text-center text-[9px] text-gray-600 mt-3 font-medium italic">
        *Draws automatically at {maxT.toLocaleString()} tickets*
      </p>
    </motion.div>
  );
}
