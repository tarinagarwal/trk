...
export default function RegistrationPanel() {
    ...
    const spotsLeft = isTier1 ? (tier1Limit - currentTotalUsers) : isTier2 ? (tier1Limit + tier2Limit - currentTotalUsers) : 0;
    ...
    }, [isSuccess, myNewCode, userAddress, resolvedAddress, hash, refetchMyCode, refetchUserData]);
    ...
    const resolveTRKCode = async (trkCode: string): Promise<{ address: string | null; error?: string }> => {
        ...
        try {
            ...
            return { 
                address: null, 
                error: isConnectionError ? 'Connection error. Check network/tunnel.' : 'Code lookup failed. Are you on the right network?'
            };
        }
    };
    ...
    const checkReferral = async (input: string) => {
        ...
        if (!target) {
            setIsValid(false); setResolvedAddress(''); setValidationError('Referral address is required'); setIsChecking(false);
            return;
        }

        setIsChecking(true); setIsValid(false); setResolvedAddress(''); setValidationError('');
        ...
        if (target.startsWith('TRK') && target.length === 8) {
            ...
            if (userAddress && walletAddress.toLowerCase() === userAddress.toLowerCase()) {
                setIsValid(false); setValidationError('Cannot refer yourself'); setIsChecking(false);
                return;
            }
            setResolvedAddress(walletAddress); setIsValid(true); setIsChecking(false);
            return;
        }

        if (!isAddress(target)) {
            setIsValid(false); setValidationError('Invalid format'); setIsChecking(false);
            return;
        }
        ...
        setResolvedAddress(target); setIsValid(true); setIsChecking(false);
    };
    ...
    if (isSuccess) {
        ...
        const creditedPracticeBalance = newUserInfo?.practiceBalance ?? newUserInfo?.[4] ?? null;
        const displayBonus = creditedPracticeBalance != null
            ? formatUnits(creditedPracticeBalance as bigint, 18)
            : isBonusAvailable ? formatUnits(currentBonus, 18) : null;
            ...
            <div className="bg-surface/90 backdrop-blur-md border border-green-500/50 p-8 rounded-2xl shadow-2xl w-full max-w-md text-center animate-in fade-in zoom-in duration-500">
                ...
                {displayBonus && Number(displayBonus) > 0 && (
                    <div className="bg-green-500/10 border border-green-500/40 rounded-xl p-4 mb-4">
                        <p className="text-xs text-green-400 uppercase tracking-widest mb-1">Sign Up Bonus Credited</p>
                        <p className="text-3xl font-mono font-black text-green-400">
                            +{Number(displayBonus).toFixed(2)} USDT
                        </p>
                        <p className="text-[10px] text-gray-400 mt-1">Added to your Practice Balance</p>
                    </div>
                )}

                <div className="bg-black/40 rounded-xl p-4 border border-gray-700 mb-6">
                    <p className="text-sm text-gray-500 uppercase tracking-widest mb-1">Your Referral Code</p>
                    <p className="text-3xl font-mono font-bold text-yellow-400 tracking-wider">
                        {myNewCode ? (myNewCode as string) : <span className="animate-pulse">Loading...</span>}
                    </p>
                </div>
    ...
    return (
        <div className="bg-surface/80 backdrop-blur-md border border-gray-800 p-8 rounded-2xl shadow-2xl w-full max-w-md relative overflow-hidden">
            ...
            <div className="text-center mb-8">
                ...
                <p className="text-gray-400 text-sm">Register to unlock your referral code and rewards.</p>
                <div className="mt-1 text-[10px] text-gray-500 font-bold uppercase tracking-widest">
                    Activation: {formatUnits(config?.onboarding.minActivation ?? BigInt(10000000000000000000), 18)} USDT
                </div>
                {isBonusAvailable && (
                     <div className="mt-2 text-[10px] text-yellow-500/80 uppercase tracking-widest">
                        {spotsLeft.toLocaleString()} {isTier1 ? 'TIER-1' : 'TIER-2'} Bonus Spots Left
                     </div>
                )}
            </div>
            ...
            <div className="flex flex-col gap-4">
                <div className="flex flex-col gap-2">
                    ...
                    <div className="relative">
                        <input
                            ...
                            className={`bg-black/50 border ${isValid ? 'border-green-500/50' : 'border-gray-700'} rounded-xl p-4 text-white font-mono text-sm w-full outline-none focus:border-primary transition-all shadow-inner`}
                            placeholder={isOwner ? "Admin registration" : "Enter Referral Code (TRK...)"}
                        />
                        {isChecking && <div className="absolute right-4 top-4 text-xs text-yellow-500 animate-pulse">Checking...</div>}
                        {isValid && !isChecking && <div className="absolute right-4 top-4 text-xs text-green-500">✓ Valid</div>}
                    </div>
                    {isValid && resolvedAddress && (
                         <p className="text-xs text-green-500/80 px-1">
                            {resolvedAddress === '0x0000000000000000000000000000000000000000' 
                                ? '✨ Admin Registration' 
                                : `Referrer: ${resolvedAddress.slice(0, 6)}...${resolvedAddress.slice(-4)}`}
                        </p>
                    )}
                ...
                >
                    {isPending ? 'Check Wallet...' : isConfirming ? 'Registering...' : 
                        isBonusAvailable ? `Register & Claim ${formatUnits(currentBonus, 18)} USDT Bonus` : 'Register Account'}
                </button>

                <div className="text-center text-xs text-gray-500 mt-2">
                    {isConfirming && <p className="text-yellow-500 animate-pulse">Waiting for network confirmation...</p>}
                </div>
            </div>
        </div>
    );
