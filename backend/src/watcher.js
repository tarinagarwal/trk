...
async function syncUser(address, referrer, userId) {
  ...
  try {
    await prisma.user.upsert({
      where: { address: addr },
      update: { isRegistered: true, referralCode: userId.toString(), referrer: ref },
      create: { address: addr, isRegistered: true, referralCode: userId.toString(), referrer: ref }
    });
  } catch (e) {
...
async function watchEvents() {
  ...
  const eventConfigs = [
    // Registry
    { address: process.env.REGISTRY_ADDRESS, abi: ABIS.REGISTRY[0], name: 'UserRegistered' },
    { address: process.env.REGISTRY_ADDRESS, abi: ABIS.REGISTRY[1], name: 'IncomeReceived' },
    { address: process.env.REGISTRY_ADDRESS, abi: ABIS.REGISTRY[2], name: 'PracticeRewardReceived' },
    // Game
    { address: process.env.GAME_ADDRESS, abi: ABIS.GAME[0], name: 'TicketsBought' },
    ...
    { address: process.env.GAME_ADDRESS, abi: ABIS.GAME[3], name: 'RoundClosed' },
    { address: process.env.LUCKY_DRAW_ADDRESS, abi: ABIS.GAME[4], name: 'DrawExecuted' },
    // Treasury
    { address: process.env.TREASURY_ADDRESS, abi: ABIS.TREASURY[0], name: 'Deposit' },
    { address: process.env.TREASURY_ADDRESS, abi: ABIS.TREASURY[1], name: 'Withdraw' }
  ];
...
async function handleLogs(logs, eventName) {
  for (const log of logs) {
    ...
    console.log(`🔔 Event Caught: ${eventName} | Tx: ${transactionHash.slice(0, 10)}...`);
      ...
      switch (eventName) {
        case 'UserRegistered':
          ...
          await prisma.registration.upsert({
            where: { hash_logIndex: { hash: transactionHash, logIndex: log.logIndex } },
            update: { timestamp: ts },
            create: { address: args.user.toLowerCase(), referrer: args.referrer.toLowerCase(), userId: Number(args.userId), hash: transactionHash, logIndex: log.logIndex, timestamp: ts }
          });
          break;
        case 'IncomeReceived':
        ...
        case 'TicketsBought':
          ...
          const field = Number(args.drawType) === 1 ? 'totalTicketsGolden' : 'totalTicketsSilver';
